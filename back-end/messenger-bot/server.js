...
app.post("/changesystemstate", (req, res) => {
  ...
  const currentState = handlesysteminformation.changeSystemState(systemState);
  res.send({ systemState: currentState });
});
...
app.post("/changetoken", (req, res) => {
  ...
  res.status(200).send({ PAGE_ACCESS_TOKEN, VERIFY_TOKEN });
});
...
app.get("/getsystemstate", (req, res) => {
  ...
  res.send({ systemState: systemState });
});

// get PAGE_ACCESS_TOKEN and VERIFY_TOKEN in .env
app.get("/gettoken", (req, res) => {
  console.log("/gettoken -- get");
  const { PAGE_ACCESS_TOKEN, VERIFY_TOKEN } =
    handlesysteminformation.getToken();
